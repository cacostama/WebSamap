...
if (isset($_SESSION['ADM_Username'])){
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2") && samap_puede_escribir() && samap_csrf_validar()) {
		...
		try {
			...
			$new_id = 0;
			$stmt = $conexion->prepare('INSERT INTO tbl_convenios (ciudad, titulo, detalle, imagen, url) VALUES (?, ?, ?, ?, ?)');
			if ($stmt) {
				$stmt->bind_param('sssss', $ciudad, $nombre, $detalle, $IMAGEN, $url);
				$stmt->execute();
...
<meta http-equiv="Content-Language" content="es"/>
	...
	<title>AGREGAR CONVENIO -  Administrador</title>
							...
							<?php echo samap_csrf_field(); ?>
								...
								<fieldset>
									<div class="form-group">
										...
										<div class="col-lg-10">
											<input type="text" name="ciudad" placeholder="" value=""  class="form-control">											
										</div>
