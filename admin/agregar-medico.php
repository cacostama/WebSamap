...
if (isset($_SESSION['ADM_Username'])){
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2") && samap_puede_escribir() && samap_csrf_validar()) {
		...
		try {
			...
			$new_id = 0;
			$stmt = $conexion->prepare('INSERT INTO tbl_medicos (titulo, nombre, especialidad, imagen) VALUES (?, ?, ?, ?)');
			if ($stmt) {
				$stmt->bind_param('ssss', $titulo, $nombre, $especialidad, $IMAGEN);
				$stmt->execute();
...
<meta http-equiv="Content-Language" content="es"/>
	...
	<title>AGREGAR MEDICO -  Administrador</title>
							...
							<?php echo samap_csrf_field(); ?>
								...
								<fieldset>
									<div class="form-group">
										...
										<div class="col-lg-10">
											<input type="text" name="especialidad" placeholder="" value=""  class="form-control">											
										</div>
								...
								<fieldset>
									...
									$upload_campo      = 'imagen';
									$upload_label      = 'Imagen';
									$upload_subcarpeta = 'medicos';
									$upload_ruta       = $rutaMedico;
